...
{
    NSString* fullURL = [NSString stringWithFormat:URL_WEIBO_DETAIL,_tid,[SzkAPI getAuthkeyGBK],pageCount];
        ...
        if ([@"0" isEqualToString:errcode])
        {
            ...
            }else
            {
                ...
                for (i = 0; i < count; i++)
                {
                    ...
                    [obj setDateline:[ZSNApi timestamp:[value objectForKey:@"dateline"]]];
...
-(void)loadWeiBoData
{
    NSString * fullURL= [NSString stringWithFormat:@"http://fb.fblife.com/openapi/index.php?mod=getweibo&code=weibocomment&tid=%@&fromtype=b5eeec0b&authkey=%@&page=1&fbtype=json",_tid,[SzkAPI getAuthkeyGBK]];
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    _DateLine_Label = [[UILabel alloc] initWithFrame:CGRectMake(DEVICE_WIDTH-100,8,80,20)];
    ...
    if (self.dataArray.count > 0)
    {
        ...
        ReplysFeed * info = [self.dataArray objectAtIndex:indexPath.row];
        [_Head_ImageView loadImageFromURL:info.faceOriginal withPlaceholdImage:PERSONAL_DEFAULTS_IMAGE];
...
-(UIView *)returnWeiBocontentView
{
    ...
    [headerView loadImageFromURL:self.info.face_original_url withPlaceholdImage:PERSONAL_DEFAULTS_IMAGE];
    ...
    if (self.info.rootFlg)
    {
        WeiBoSpecialView * _content_reply_special = [[WeiBoSpecialView alloc] initWithFrame:CGRectMake(8,10,DEVICE_WIDTH-80,0)];
...
{
    FBCircleWebViewController * web = [[FBCircleWebViewController alloc]init];
    web.web_url = isRe?self.info.rolink:self.info.olink;
...
{
    FBCircleWebViewController *fbweb=[[FBCircleWebViewController alloc]init];
    fbweb.web_url = theUrl;
